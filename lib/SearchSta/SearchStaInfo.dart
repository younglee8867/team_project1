        ...
        children: [
          _buildDetailText(
              ...
              stationDetails['schedule']?['interval'] != null
                  ? '${stationDetails['schedule']?['interval']}' +
                      " " +
                      '분'.tr()
                  : '정보 없음'),
          SizedBox(width: 20),
    ...
    return _buildSection(
      ...
      content: Column(
        children: [
          Row(children: [
            _buildDetailText(
                '내리는문'.tr(), facilityInfo['doorSide'].toString().tr()),
            SizedBox(width: 100),
            _buildDetailText('화장실'.tr(), facilityInfo['restrooms']),
          ]),
          Row(children: [
            _buildDetailText(
                '플랫폼'.tr(), facilityInfo['platformType'].toString().tr()),
          ]),
        ],
      ),
    );
...
// 편의시설 위젯
  Widget _buildDetailTextAmenities(String label, bool value) {
    ...
    final textValue = value
        ...
        : TextStyle(
            color: themeNotifier.isDarkMode
                ? const Color.fromARGB(240, 173, 173, 173)
